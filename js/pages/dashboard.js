...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function initDashboard(user) {
        ...
        // 2. Latest Test Score
        fetch('backend/exams/get_latest_score.php')
            .then(res => res.json())
            .then(data => {
                const scoreVal = document.getElementById('latest-score-val');
                const scoreDesc = document.getElementById('latest-score-desc');

                if (data.success && data.latest) {
                    const latest = data.latest;
                    scoreVal.innerText = `${Math.round(latest.percentage)}%`;
                    scoreDesc.innerText = `${latest.date} • ${latest.score}/${latest.total}`;
                } else {
                    // Fallback to session data for immediate updates after a test
                    const sessionTest = window.authApp.getUserData('latest_test_score');
                    if (sessionTest) {
                        scoreVal.innerText = `${sessionTest.percentage}%`;
                        scoreDesc.innerText = `${sessionTest.date} • ${sessionTest.score}/${sessionTest.total}`;
                    } else {
                        scoreVal.innerText = 'No test results yet';
                        scoreDesc.innerText = 'Complete your first practice test to track your progress.';
                    }
                }
            })
            .catch(err => {
                ...
                const sessionTest = window.authApp.getUserData('latest_test_score');
                if (sessionTest) {
                    document.getElementById('latest-score-val').innerText = `${sessionTest.percentage}%`;
                    document.getElementById('latest-score-desc').innerText = `${sessionTest.date} • ${sessionTest.score}/${sessionTest.total}`;
                }
            });
        ...
        .then(([progressData, statsData]) => {
            const studyProgress = (progressData.success && progressData.progress) ? progressData.progress : (window.authApp.getUserData('study_progress') || {});
            
            // Determine practice test status from passing stats
            if (statsData.success) {
                if (statsData.total_passed >= 3) {
                    studyProgress['practice-test'] = 'completed';
                } else if (statsData.total_attempted > 0) {
                    studyProgress['practice-test'] = 'in-progress';
                } else {
                    studyProgress['practice-test'] = 'not-started';
                }
            }
                const totalTopics = 5;
                ...
                if (percentVal) percentVal.innerText = `${progressPercent}%`;
                if (fill) {
